...
import init, { Konsh } from "portfolio";
...
class Main extends React.Component {
    ...
    processCommand(command) {
        ...
        if (com.length === 0) {
            ...
            let b = this.konsh.decode(command).map((val) => {
                let s = val.split("\n");
                return <span className={s[1]}>{s[0]}</span>;
            });
            console.log(b);
            return b;
        }
